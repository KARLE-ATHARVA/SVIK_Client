...
//   scene: THREE.Scene;
//   onFloorMaterialReady: (material: THREE.MeshStandardMaterial) => void;
//   onWallMaterialsReady: (materials: any[]) => void;
// }
...
//   const sofaVelvet    = new THREE.MeshStandardMaterial({ color: 0x1e3a5f, roughness: 0.85, metalness: 0.05 });
//   const cushionMat    = new THREE.MeshStandardMaterial({ color: 0xf2ead8, roughness: 0.9,  metalness: 0.0  });
//   const throwMat      = new THREE.MeshStandardMaterial({ color: 0xb35a35, roughness: 0.95, metalness: 0.0, side: THREE.DoubleSide });
//   const darkWoodMat   = new THREE.MeshStandardMaterial({ color: 0x2d1a0e, roughness: 0.55, metalness: 0.04 });
//   const lightWoodMat  = new THREE.MeshStandardMaterial({ color: 0x8b6340, roughness: 0.62, metalness: 0.04 });
//   const goldMat       = new THREE.MeshStandardMaterial({ color: 0xc8960c, roughness: 0.2,  metalness: 0.9, envMapIntensity: 1.5 });
//   const blackMetalMat = new THREE.MeshStandardMaterial({ color: 0x1a1a1c, roughness: 0.3,  metalness: 0.85 });
//   const marbletopMat  = new THREE.MeshStandardMaterial({ color: 0xe8e2d8, roughness: 0.18, metalness: 0.02, envMapIntensity: 0.9 });
//   const tvFrameMat    = new THREE.MeshStandardMaterial({ color: 0x0d0d0d, roughness: 0.25, metalness: 0.7  });
//   const tvScreenMat   = new THREE.MeshStandardMaterial({ color: 0x080e18, roughness: 0.08, metalness: 0.6, emissive: 0x1a3a6a, emissiveIntensity: 0.8 });

//   const glassMat = new THREE.MeshPhysicalMaterial({
//     color: 0xc8e0f5, roughness: 0.0, metalness: 0.0,
//     transparent: true, opacity: 0.15, transmission: 0, ior: 1.52,
...
//     emissive: new THREE.Color(0xff9922), emissiveIntensity: 0.5,
//   });

//   const pendantShadeMat = new THREE.MeshStandardMaterial({
//     color: 0x1a1a1a, roughness: 0.35, metalness: 0.65, side: THREE.DoubleSide,
//     emissive: new THREE.Color(0xfff0d0), emissiveIntensity: 0.4,
//   });

//   const tubeMat1 = new THREE.MeshStandardMaterial({
//     color: 0xffffff, roughness: 0.12, metalness: 0.1,
//     emissive: new THREE.Color(0xfff8f0), emissiveIntensity: 0.6,
//   });
//   const tubeMat2 = new THREE.MeshStandardMaterial({
//     color: 0xffffff, roughness: 0.12, metalness: 0.1,
//     emissive: new THREE.Color(0xfff8f0), emissiveIntensity: 0.6,
...
//   // FIX: symmetric positions matching kitchen pattern (-4.2 left, +4.2 right)
//   const tubeLight1 = new THREE.PointLight(0xfff8f0, 4.0, 14);
//   tubeLight1.position.set(-7.8, 5.6, -1);
//   tubeLight1.castShadow = true; tubeLight1.shadow.mapSize.set(512, 512);
//   scene.add(tubeLight1);

//   const tubeLight2 = new THREE.PointLight(0xfff8f0, 4.0, 14);
//   tubeLight2.position.set(7.8, 5.6, -1);
//   tubeLight2.castShadow = true; tubeLight2.shadow.mapSize.set(512, 512);
//   scene.add(tubeLight2);
...
//   // LIGHT TOGGLE STATE
//   // ==============================================
//   const lightStates = { floor: true, pendant: true, tube1: true, tube2: true };
//   const defaults = {
//     floorLamp: { light: 2.5, shade: 0.5,  emissive: 0xff9922 as number },
//     pendant:   { light: 3.0, shade: 0.4,  emissive: 0xfff0d0 as number },
//     tube1:     { light: 4.0, emissive: 0.6 },
...
//   const backWall = new THREE.Mesh(new THREE.PlaneGeometry(20, 7), backWallMat);
//   backWall.position.set(0, 3.5, -8); backWall.receiveShadow = true;
//   backWall.name = SURFACE_NAMES.wallBack; scene.add(backWall);

//   const leftWall = new THREE.Mesh(new THREE.PlaneGeometry(16, 7), leftWallMat);
//   leftWall.rotation.y = Math.PI / 2; leftWall.position.set(-10, 3.5, 0);
//   leftWall.receiveShadow = true; leftWall.name = SURFACE_NAMES.wallLeft; scene.add(leftWall);

//   const rightWall = new THREE.Mesh(new THREE.PlaneGeometry(16, 7), rightWallMat);
//   rightWall.rotation.y = -Math.PI / 2; rightWall.position.set(10, 3.5, 0);
...
//   const frameMat = new THREE.MeshStandardMaterial({ color: 0x1a1a1a, roughness: 0.35, metalness: 0.75 });
//   const curtainMat = new THREE.MeshStandardMaterial({
//     color: 0xd8c8a0, roughness: 0.95, metalness: 0.0, transparent: true, opacity: 0.45, side: THREE.DoubleSide,
//   });
//   const windowMaskMat = new THREE.MeshStandardMaterial({
//     color: 0xffffff, roughness: 0.9, metalness: 0.0,
//     emissive: new THREE.Color(0xffffff), emissiveIntensity: 0.05, side: THREE.FrontSide,
//   });
...
//     const maskPanel = new THREE.Mesh(new THREE.PlaneGeometry(W + 0.6, H + 0.7), windowMaskMat);
//     maskPanel.rotation.y = side > 0 ? Math.PI / 2 : -Math.PI / 2;
//     maskPanel.position.set(side * 0.04, 0, 0); maskPanel.renderOrder = 0; g.add(maskPanel);

//     const sky = new THREE.Mesh(new THREE.PlaneGeometry(W - FT * 2, H * 0.55),
//       new THREE.MeshBasicMaterial({ color: 0x08102a, side: THREE.DoubleSide }));
//     sky.rotation.y = side > 0 ? Math.PI / 2 : -Math.PI / 2;
//     sky.position.set(OFF - side * 0.04, H * 0.225, 0); sky.renderOrder = 1; g.add(sky);

//     const city = new THREE.Mesh(new THREE.PlaneGeometry(W - FT * 2, H * 0.45),
//       new THREE.MeshBasicMaterial({ color: 0x1a2840, side: THREE.DoubleSide }));
//     city.rotation.y = side > 0 ? Math.PI / 2 : -Math.PI / 2;
//     city.position.set(OFF - side * 0.04, -H * 0.225, 0); city.renderOrder = 1; g.add(city);

//     const cityGlow = new THREE.PointLight(0x3060c0, 1.5, 5);
//     cityGlow.position.set(OFF - side * 0.3, 0, 0); g.add(cityGlow);

//     const glass = new THREE.Mesh(new THREE.PlaneGeometry(W, H), glassMat);
//     glass.rotation.y = side > 0 ? Math.PI / 2 : -Math.PI / 2;
//     glass.position.set(OFF, 0, 0); glass.renderOrder = 2; g.add(glass);
...
//     const rod = new THREE.Mesh(new THREE.CylinderGeometry(0.016, 0.016, W + 1.0, 16), goldMat);
//     rod.rotation.z = Math.PI / 2;
//     rod.position.set(OFF + side * 0.06, H / 2 + 0.22, 0); g.add(rod);
//     for (const dz of [-(W + 1.0) / 2, (W + 1.0) / 2]) {
//       const fin = new THREE.Mesh(new THREE.SphereGeometry(0.03, 10, 10), goldMat);
//       fin.position.set(OFF + side * 0.06, H / 2 + 0.22, dz); g.add(fin);
//     }

//     for (const sign of [-1, 1]) {
//       const curt = new THREE.Mesh(new THREE.PlaneGeometry(0.38, H + 0.55), curtainMat);
//       curt.rotation.y = side > 0 ? Math.PI / 2 : -Math.PI / 2;
//       curt.position.set(OFF + side * 0.04, 0.1, sign * (W / 2 + 0.2)); g.add(curt);
//     }
...
//   // ==============================================
//   // TUBE LIGHT FIXTURES (ceiling)
//   // FIX: symmetric at -4.2 / +4.2 matching kitchen
//   // FIX: proxy at eye level y=1.8, radius=1.0 for easy clicking
//   // ==============================================
//   function makeTubeFixture(px: number, pz: number, diffMat: THREE.MeshStandardMaterial, lightName: string) {
//     const g = new THREE.Group();
...
//     const diffuser = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.04, 1.85, 16), diffMat);
//     diffuser.rotation.z = Math.PI / 2;
//     diffuser.position.y = -0.07;
//     diffuser.name = `${lightName}_visual`;
//     g.add(diffuser);
...
//         new THREE.MeshStandardMaterial({ color: 0xa0a0a8, metalness: 0.8, roughness: 0.25 })
//       );
//       cap.rotation.z = Math.PI / 2; cap.position.set(sx, -0.07, 0); g.add(cap);
//     }

//     g.position.set(px, 6.97, pz);
//     scene.add(g);

//     // FIX: proxy at y=1.8 (eye/reach level), radius=1.0 for easy clicking
//     const proxy = new THREE.Mesh(
//       new THREE.SphereGeometry(1.0, 8, 8),
//       new THREE.MeshBasicMaterial({ transparent: true, opacity: 0, depthWrite: false })
//     );
//     proxy.name = lightName;
//     proxy.position.set(px, 1.8, pz);
//     scene.add(proxy);
//   }

//   makeTubeFixture(-4.2, -1, tubeMat1, LIVING_LIGHT_NAMES.tubeLight1);
//   makeTubeFixture( 4.2, -1, tubeMat2, LIVING_LIGHT_NAMES.tubeLight2);
...
//   function addSofaLeg(x: number, z: number) {
//     const leg = new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.12, 0.06), legMat);
//     leg.position.set(x, 0.06, z); sofaGroup.add(leg);
//   }
...
//   const throwBlanket = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.06, 0.8), throwMat);
//   throwBlanket.position.set(1.5, 0.72, 0.1); throwBlanket.rotation.z = 0.3; sofaGroup.add(throwBlanket);

//   addSofaLeg(-1.62, -0.48); addSofaLeg(1.62, -0.48);
//   addSofaLeg(-1.62,  0.48); addSofaLeg(1.62,  0.48);

//   sofaGroup.position.set(-4, 0, 3.2); sofaGroup.rotation.y = Math.PI;
//   markMovable(sofaGroup, 'furniture_sofa', 'Sofa');
//   scene.add(sofaGroup);
...
//     cLeg.position.set(x, 0.175, z); chairGroup.add(cLeg);
//   }
//   chairGroup.position.set(-1.5, 0, 0.5); chairGroup.rotation.y = -Math.PI / 8;
//   markMovable(chairGroup, 'furniture_accent_chair', 'Accent Chair');
//   scene.add(chairGroup);
...
//       new THREE.MeshStandardMaterial({ color: 0xc0b090, roughness: 0.2 })
//     );
//     vein.rotation.x = -Math.PI / 2; vein.rotation.z = (i * 1.2); vein.position.y = 0.48;
//     tableGroup.add(vein);
//   }
//   const pedestal = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.18, 0.42, 24), goldMat);
//   pedestal.position.y = 0.21; tableGroup.add(pedestal);
...
//   const coffeeBook = new THREE.Mesh(new THREE.BoxGeometry(0.3, 0.03, 0.22), bookMat);
//   coffeeBook.position.set(-0.2, 0.5, -0.1); tableGroup.add(coffeeBook);
//   const smallVase = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.03, 0.18, 16),
//     new THREE.MeshStandardMaterial({ color: 0x1a3a2a, roughness: 0.2, metalness: 0.3 }));
//   smallVase.position.set(0.3, 0.57, 0.1); tableGroup.add(smallVase);
...
//     bracket.position.set(x, 0.125, 0); tvGroup.add(bracket);
//     const pull = new THREE.Mesh(new THREE.CylinderGeometry(0.015, 0.015, 0.2, 8), goldMat);
//     pull.rotation.z = Math.PI / 2; pull.position.set(x, 0.5, 0.28); tvGroup.add(pull);
//   }
//   const tvFrame = new THREE.Mesh(new THREE.BoxGeometry(3.0, 1.7, 0.06), tvFrameMat);
//   tvFrame.position.set(0, 1.85, -0.02); tvGroup.add(tvFrame);
//   const tvScreen = new THREE.Mesh(new THREE.PlaneGeometry(2.85, 1.58), tvScreenMat);
//   tvScreen.position.set(0, 1.85, 0.04); tvGroup.add(tvScreen);
//   const tvLogo = new THREE.Mesh(new THREE.BoxGeometry(0.15, 0.015, 0.01),
//     new THREE.MeshStandardMaterial({ color: 0x888888, roughness: 0.4, metalness: 0.6 }));
//   tvLogo.position.set(0, 1.0, 0.04); tvGroup.add(tvLogo);
//   tvGroup.position.set(-4, 0.25, -7.4);
//   markMovable(tvGroup, 'furniture_tv', 'TV & Console');
...
//       const w = 0.1 + Math.random() * 0.12;
//       const h = 0.28 + Math.random() * 0.24;
//       const book = new THREE.Mesh(new THREE.BoxGeometry(w, h, 0.24),
//         new THREE.MeshStandardMaterial({ color: bookColors2[Math.floor(Math.random() * bookColors2.length)], roughness: 0.85 }));
//       book.position.set(bx + w / 2, s * 0.6 + 0.18 + h / 2, 0.18);
//       book.rotation.y = (Math.random() - 0.5) * 0.06;
//       book.castShadow = true; bookshelf.add(book);
//       bx += w + 0.01;
//     }
//     if (s % 2 === 0) {
//       const deco = new THREE.Mesh(new THREE.CylinderGeometry(0.055, 0.04, 0.2, 16),
//         new THREE.MeshStandardMaterial({ color: 0xb8860b, roughness: 0.25, metalness: 0.75 }));
//       deco.position.set(0.75, s * 0.6 + 0.22, 0.18); bookshelf.add(deco);
//     }
//   }
//   bookshelf.position.set(-8.7, 0, -5.5); bookshelf.rotation.y = Math.PI / 2;
//   markMovable(bookshelf, 'furniture_bookshelf', 'Bookshelf');
//   scene.add(bookshelf);
...
//     const tleg = new THREE.Mesh(new THREE.CylinderGeometry(0.018, 0.014, 0.6, 12), goldMat);
//     tleg.position.set(Math.cos(a) * 0.22, 0.3, Math.sin(a) * 0.22);
//     tleg.rotation.z = Math.sin(a) * 0.1; sideTableGroup.add(tleg);
//   }
//   const tlampPole = new THREE.Mesh(new THREE.CylinderGeometry(0.018, 0.018, 0.35, 12), goldMat);
//   tlampPole.position.y = 0.82; sideTableGroup.add(tlampPole);
//   const tlampShade = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.13, 0.22, 24, 1, true),
//     new THREE.MeshStandardMaterial({
//       color: 0xf5e8d0, roughness: 0.9, side: THREE.DoubleSide,
//       transparent: true, opacity: 0.88, emissive: 0xff9922, emissiveIntensity: 1.5,
//     }));
//   tlampShade.position.y = 1.05; sideTableGroup.add(tlampShade);
//   const tableLampLight = new THREE.PointLight(0xff8822, 8, 4.5);
//   tableLampLight.position.y = 1.1; sideTableGroup.add(tableLampLight);
//   const coaster = new THREE.Mesh(new THREE.CylinderGeometry(0.09, 0.09, 0.008, 20),
//     new THREE.MeshStandardMaterial({ color: 0x3a2010, roughness: 0.9 }));
//   coaster.position.y = 0.648; sideTableGroup.add(coaster);
//   const mug = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.035, 0.09, 16),
...
//     const t = i / 7;
//     const seg = new THREE.Mesh(new THREE.CylinderGeometry(0.016, 0.016, 0.32, 10), poleMat);
//     seg.position.set(t * 0.7, 0.16 + t * 1.8, 0); seg.rotation.z = -t * 0.5; lampGroup.add(seg);
//   }
//   const arcShade = new THREE.Mesh(
//     new THREE.CylinderGeometry(0.32, 0.28, 0.38, 32, 1, true), floorShadeMat);
//   arcShade.position.set(0.65, 2.05, 0); arcShade.castShadow = true;
//   arcShade.name = LIVING_LIGHT_NAMES.floorShade; lampGroup.add(arcShade);
//   const innerDiff = new THREE.Mesh(new THREE.CylinderGeometry(0.28, 0.24, 0.3, 32, 1, false),
//     new THREE.MeshStandardMaterial({ color: 0xfff8e8, roughness: 0.9, transparent: true, opacity: 0.6 }));
//   innerDiff.position.set(0.65, 2.05, 0); lampGroup.add(innerDiff);
//   lampGroup.position.set(-7.2, 0, 3.5);
//   markMovable(lampGroup, 'furniture_floor_lamp', 'Floor Lamp');
...
//   // FIX: proxy world pos = lampGroup.pos + shade local pos = (-7.2+0.65, 2.05, 3.5)
//   // radius 0.65 so the whole shade area is clickable
//   const floorLampProxy = new THREE.Mesh(
...
//   // WALL ART
//   // ==============================================
//   const artFrameLarge = new THREE.Mesh(new THREE.BoxGeometry(2.0, 1.5, 0.06),
//     new THREE.MeshStandardMaterial({ color: 0x1a1a1a, roughness: 0.6 }));
//   artFrameLarge.position.set(3.5, 3.0, -7.92); scene.add(artFrameLarge);
//   const artCanvas1 = new THREE.Mesh(new THREE.PlaneGeometry(1.8, 1.3),
//     new THREE.MeshStandardMaterial({ color: 0x2a3a5a, roughness: 0.7 }));
//   artCanvas1.position.set(3.5, 3.0, -7.88); scene.add(artCanvas1);
//   for (const [cx, cy, col] of [[3.2, 3.2, 0xc84040], [3.8, 2.8, 0xe0a030], [3.5, 3.0, 0xf0f0f0]]) {
//     const cb = new THREE.Mesh(new THREE.PlaneGeometry(0.35, 0.3),
//       new THREE.MeshStandardMaterial({ color: col as number, roughness: 0.8 }));
//     cb.position.set(cx as number, cy as number, -7.86); scene.add(cb);
//   }
//   const picLight = new THREE.SpotLight(0xffe8c0, 15, 3, Math.PI / 10, 0.5);
//   picLight.position.set(3.5, 4.2, -7.5);
//   picLight.target.position.set(3.5, 3.0, -7.9);
...
//   const photoFrame = new THREE.Mesh(new THREE.BoxGeometry(0.7, 0.85, 0.04),
//     new THREE.MeshStandardMaterial({ color: 0xb8860b, roughness: 0.3, metalness: 0.7 }));
//   photoFrame.position.set(6.0, 2.8, -7.92); scene.add(photoFrame);
//   const photoCanvas = new THREE.Mesh(new THREE.PlaneGeometry(0.56, 0.72),
//     new THREE.MeshStandardMaterial({ color: 0x6a5a48, roughness: 0.8 }));
//   photoCanvas.position.set(6.0, 2.8, -7.88); scene.add(photoCanvas);

//   const mirrorRim = new THREE.Mesh(new THREE.TorusGeometry(0.65, 0.06, 12, 48), goldMat);
//   mirrorRim.position.set(7.5, 2.8, -7.92); scene.add(mirrorRim);
//   const mirrorGlass = new THREE.Mesh(new THREE.CircleGeometry(0.63, 48),
//     new THREE.MeshStandardMaterial({ color: 0xd8e8f0, roughness: 0.0, metalness: 0.9, envMapIntensity: 2.0 }));
//   mirrorGlass.position.set(7.5, 2.8, -7.89); scene.add(mirrorGlass);
...
//   pot.position.y = 0.19; plantGroup.add(pot);
//   const potRim = new THREE.Mesh(new THREE.TorusGeometry(0.225, 0.015, 8, 32), potMat);
//   potRim.rotation.x = Math.PI / 2; potRim.position.y = 0.38; plantGroup.add(potRim);
//   const soilMesh = new THREE.Mesh(new THREE.CircleGeometry(0.21, 24),
//     new THREE.MeshStandardMaterial({ color: 0x2a1a0a, roughness: 0.98 }));
//   soilMesh.rotation.x = -Math.PI / 2; soilMesh.position.y = 0.385; plantGroup.add(soilMesh);
//   const trunk = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.05, 1.2, 12), lightWoodMat);
//   trunk.position.y = 0.98; trunk.rotation.z = 0.05; plantGroup.add(trunk);
...
//     const leaf = new THREE.Mesh(leafGeo, leafMat);
//     leaf.position.set(Math.cos(a) * 0.2, 1.2 + h * 0.6, Math.sin(a) * 0.2);
//     leaf.rotation.y = a; leaf.rotation.z = -0.3; leaf.castShadow = true; plantGroup.add(leaf);
//   }
//   plantGroup.position.set(8.5, 0, 5.5);
//   markMovable(plantGroup, 'furniture_plant', 'Fiddle Leaf Plant');
//   scene.add(plantGroup);
...
//   addChair( 0,   1.5,  Math.PI);
//   addChair( 0,  -1.7,  0);
//   const centerpiece = new THREE.Mesh(new THREE.CylinderGeometry(0.15, 0.12, 0.25, 24),
//     new THREE.MeshStandardMaterial({ color: 0x8b4513, roughness: 0.6, metalness: 0.2 }));
//   centerpiece.position.set(0, 0.92, -0.5); centerpiece.castShadow = true; diningGroup.add(centerpiece);
//   diningGroup.position.set(5, 0, 0);
//   markMovable(diningGroup, 'furniture_dining_set', 'Dining Set');
...
//   // PENDANT LAMP above dining table
//   // ==============================================
//   const pendantCord = new THREE.Mesh(new THREE.CylinderGeometry(0.01, 0.01, 2.2, 8),
//     new THREE.MeshStandardMaterial({ color: 0x1a1a1a, roughness: 0.6 }));
//   pendantCord.position.set(5, 5.9, 0); scene.add(pendantCord);

//   const pendantShade = new THREE.Mesh(new THREE.CylinderGeometry(0.38, 0.42, 0.28, 32), pendantShadeMat);
//   pendantShade.position.set(5, 4.6, 0); pendantShade.castShadow = true;
//   pendantShade.name = LIVING_LIGHT_NAMES.pendantShade; scene.add(pendantShade);

//   // FIX: pendant proxy at y=2.0 — reachable while rotating around room
//   const pendantProxy = new THREE.Mesh(
//     new THREE.SphereGeometry(0.65, 8, 8),
//     new THREE.MeshBasicMaterial({ transparent: true, opacity: 0, depthWrite: false })
//   );
//   pendantProxy.name = LIVING_LIGHT_NAMES.pendantShade;
//   pendantProxy.position.set(5, 2.0, 0);
//   scene.add(pendantProxy);
...
//   consoleBody.position.y = 0.41; consoleGroup.add(consoleBody);
//   const xBar1 = new THREE.Mesh(new THREE.BoxGeometry(2.0, 0.04, 0.04), goldMat);
//   xBar1.rotation.z = Math.atan2(0.7, 2.0); xBar1.position.set(0, 0.41, 0.2); consoleGroup.add(xBar1);
//   const xBar2 = xBar1.clone(); xBar2.rotation.z = -Math.atan2(0.7, 2.0); consoleGroup.add(xBar2);
//   const consoleLamp = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.04, 0.3, 16),
//     new THREE.MeshStandardMaterial({ color: 0xd4af37, roughness: 0.2, metalness: 0.85 }));
//   consoleLamp.position.set(-0.8, 1.04, 0); consoleGroup.add(consoleLamp);
//   const consolePlant = new THREE.Mesh(new THREE.SphereGeometry(0.12, 12, 12),
//     new THREE.MeshStandardMaterial({ color: 0x3a6030, roughness: 0.85 }));
//   consolePlant.position.set(0.7, 1.02, 0); consoleGroup.add(consolePlant);
//   consoleGroup.position.set(-4, 0, 1.9); consoleGroup.rotation.y = Math.PI;
//   markMovable(consoleGroup, 'furniture_console', 'Console Table');
//   scene.add(consoleGroup);
...
//   // CEILING ROSE
//   // ==============================================
//   const ceilRose = new THREE.Mesh(new THREE.CylinderGeometry(0.12, 0.12, 0.04, 24), cofferMat);
//   ceilRose.position.set(5, 6.98, 0); scene.add(ceilRose);
...
export function buildLivingRoomScene({
  ...
  // Tube lights spread wide left and right so they're visible from all angles
  const tubeLight1 = new THREE.PointLight(0xfff8f0, 4.0, 14);
  tubeLight1.position.set(-7.0, 5.6, 0);
  tubeLight1.castShadow = true; tubeLight1.shadow.mapSize.set(512, 512);
  scene.add(tubeLight1);

  const tubeLight2 = new THREE.PointLight(0xfff8f0, 4.0, 14);
  tubeLight2.position.set(7.0, 5.6, 0);
  tubeLight2.castShadow = true; tubeLight2.shadow.mapSize.set(512, 512);
  scene.add(tubeLight2);
  ...
  // ==============================================
  // TUBE LIGHT FIXTURES (ceiling)
  // Spread wide at -7.0 / +7.0 so fixtures are visible when rotating
  // Proxy at eye level y=1.8, radius=1.0 for easy clicking
  // ==============================================
  function makeTubeFixture(px: number, pz: number, diffMat: THREE.MeshStandardMaterial, lightName: string) {
    const g = new THREE.Group();
    ...
    // Proxy at y=1.8 (eye/reach level), radius=1.0 for easy clicking
    const proxy = new THREE.Mesh(
      new THREE.SphereGeometry(1.0, 8, 8),
      new THREE.MeshBasicMaterial({ transparent: true, opacity: 0, depthWrite: false })
    );
  ...
  // Fixtures at -7.0 / +7.0 matching the point light positions
  makeTubeFixture(-7.0, 0, tubeMat1, LIVING_LIGHT_NAMES.tubeLight1);
  makeTubeFixture( 7.0, 0, tubeMat2, LIVING_LIGHT_NAMES.tubeLight2);
  ...
  pendantProxy.name = LIVING_LIGHT_NAMES.pendantShade;
  pendantProxy.position.set(5, 4.6, 0);
  scene.add(pendantProxy);
