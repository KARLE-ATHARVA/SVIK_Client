...
function readWindowValue(key: string): string {
  ...
  return normalizeBase(String(Reflect.get(window, key) ?? "").trim());
}
...
function isProxyBase(value?: string | null): boolean {
...
export function getAssetBase(): string {
  ...
  if (process.env.NODE_ENV !== "development" && isProxyBase(configuredBase)) {
    return remoteBase || configuredBase;
  }

  return configuredBase || remoteBase;
}
