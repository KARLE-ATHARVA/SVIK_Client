...
$(function(){
    ...
    function resolveMailEndpoint() {
        ...
        if (
            ...
            return normalizedApiBase ? (normalizedApiBase + "/visualizermail") : "/visualizermail";
        }
        return endpoint;
    }
  ...
  function normalizeProductLink(url) {
    ...
    function getRemoteAssetBase() {
        ...
        if (typeof window.NEXT_PUBLIC_REMOTE_ASSET_BASE === "string" && window.NEXT_PUBLIC_REMOTE_ASSET_BASE.trim()) {
            base = window.NEXT_PUBLIC_REMOTE_ASSET_BASE.trim();
        } else {
            try {
                if (window.parent && window.parent !== window) {
                    var parentBase = window.parent.NEXT_PUBLIC_REMOTE_ASSET_BASE || "";
                    if (typeof parentBase === "string" && parentBase.trim()) base = parentBase.trim();
                }
            } catch (e) {}
        }
        if (!base) base = getVisualizerAssetBase();
        return base.replace(/\/+$/, "") + "/";
    }
    ...
    function loadImageForPdf(source, done) {
        if (!source) {
            done(null);
            return;
        }
        ...
        function renderLoadedImage(img) {
            try {
        ...
        if (typeof source !== "string" && source.tagName && String(source.tagName).toUpperCase() === "IMG") {
            if (source.complete && (source.naturalWidth || source.width)) {
                renderLoadedImage(source);
                return;
            }
            source.onload = function() { renderLoadedImage(source); };
            source.onerror = function() { finish(null); };
            return;
        }
        ...
        img.crossOrigin = "anonymous";
        img.onload = function() {
            renderLoadedImage(img);
        };
        img.onerror = function() { finish(null); };
        img.src = source;
    }

    function fetchQrCanvas(link, done) {
        if (!link) {
            done(null);
            return;
        }
        ...
        function normalizeCanvasToJpeg(canvas) {
            try {
                var out = document.createElement("canvas");
                out.width = canvas.width || 220;
                out.height = canvas.height || 220;
        ...
        function normalizeDataUrlToJpeg(dataUrl, cb) {
            if (!dataUrl) {
                cb(null);
                return;
            }
            ...
            img.onload = function() {
                try {
                    ...
                    cb(canvas.toDataURL("image/jpeg", 0.98));
                } catch (e) {
                    cb(null);
                }
            };
        ...
        function ensureQrLib(cb) {
            ...
            var existing = document.getElementById("qr-lib");
            if (existing) {
                try {
                    if (
                        window.QRCode &&
            ...
            script.id = "qr-lib";
            script.async = true;
            script.src = "/app/visualizer/js/qrcode-bundle.js?v=20260330c";
        ...
        ensureQrLib(function(ok) {
            ...
            try {
                if (typeof window.QRCode.toCanvas === "function") {
                    ...
                    window.QRCode.toCanvas(qrCanvas, link, options, function(err) {
                        if (err) {
                            finish(null);
                            return;
                        }
            ...
            try {
                if (typeof window.QRCode.toCanvas === "function") {
                    var qrCanvasPromise = document.createElement("canvas");
                    var maybeCanvasPromise = window.QRCode.toCanvas(qrCanvasPromise, link, options);
                    if (maybeCanvasPromise && typeof maybeCanvasPromise.then === "function") {
                        maybeCanvasPromise.then(function() {
                            finish(qrCanvasPromise);
                        }).catch(function() {
                            finish(null);
                        });
                        return;
            ...
            try {
                ...
                var maybePromise = window.QRCode.toDataURL(link, options);
                if (maybePromise && typeof maybePromise.then === "function") {
                    maybePromise.then(function(url) {
                        var img = new Image();
            ...
            window.QRCode.toDataURL(link, options, function(err, url) {
                if (err) {
                    finish(null);
                    return;
                }
    ...
    function drawProductCard(pdf, card, y, done) {
        ...
        var tileImage = (tile.image && typeof tile.image !== "string" && tile.image.tagName && String(tile.image.tagName).toUpperCase() === "IMG")
            ? tile.image
        ...
        loadImageForPdf(tileImage, function(tileImgData) {
            ...
            fetchGeneratedProductUrl(tile, function(generatedLink) {
                ...
                fetchQrCanvas(qrLink, function(qrImage) {
                if (qrImage) {
                    pdf.addImage(qrImage, "PNG", qrBox.x + 1, qrBox.y + 1, qrBox.w - 2, qrBox.h - 2);
                } else {
                    pdf.setFontSize(8);
...
function getDirectUserActivityUrl() {
    var base = (typeof readApiBase === "function") ? readApiBase() : "";
    if (!base) return "";
