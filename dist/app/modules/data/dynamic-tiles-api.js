(function(global) {
    ...
    (function syncProductionApiBase() {
        try {
            var host = (global.location && global.location.hostname) ? global.location.hostname : "";
            if (!host || host === "localhost" || host === "127.0.0.1") return;
            var stored = global.localStorage && localStorage.getItem("visualizer_api_base");
            if (stored && /localhost|127\.0\.0\.1/i.test(String(stored))) {
                localStorage.setItem("visualizer_api_base", "https://vyr.svikinfotech.in/api/");
            }
        } catch (e) {}
    })();
    ...
    function isLocalhostBase(base) {
        ...
        return s.indexOf("localhost") !== -1 || s.indexOf("127.0.0.1") !== -1;
    }

    function coerceApiBase(base) {
        if (isLocalhostBase(base)) {
            return "https://vyr.svikinfotech.in/api/";
        }
        return base;
    }
    ...
    function getApiBase(opts) {
        ...
        var picked = fromOpts || fromGlobal || fromStorage || fromPublicEnv || "https://vyr.svikinfotech.in/api/";
        var normalized = normalizeApiBase(picked);
        return normalizeApiBase(coerceApiBase(normalized));
    }
