...
const degToRad = (deg: number) => (deg * Math.PI) / 180;
  ...
  const resolveMailEndpoint = () => {
    const rawBase =
      ...
      (typeof window !== "undefined"
        ? String((window as Window & Record<string, unknown>).NEXT_PUBLIC_API_BASE ?? "").trim()
        : "");
  ...
  const handleSavePDF = async () => {
    ...
    for (const tile of allTiles) {
      ...
      const tileImgUrl = resolveAssetUrl(tile.image);
  ...
  const handleMailSend = async () => {
    ...
    try {
      ...
      formData.append("tiles", JSON.stringify(tilesPayload));
  ...
  const getTileImageSrc = (src?: string) => resolveAssetUrl(src);
