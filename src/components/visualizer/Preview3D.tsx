...
import { buildBathroomScene, BathroomRefs, TUBE_LIGHT_NAMES as BATH_TUBE_NAMES }      from "@/3d-scenes/bathroom";
import { buildBedroomScene, LAMP_NAMES, BedroomRefs }                                  from "@/3d-scenes/bedroom";
import { buildKitchenScene, KitchenRefs, TUBE_LIGHT_NAMES as KIT_TUBE_NAMES }         from "@/3d-scenes/kitchen";
import { buildLivingRoomScene, SURFACE_NAMES, LivingRoomRefs, LIVING_LIGHT_NAMES }    from "@/3d-scenes/living_room";
...
function getRepeat(size: string | null, surface: "floor" | "wall"): [number, number] {
  const s = (size ?? "").trim().toUpperCase().replace(/\s+/g, "");
  if (surface === "floor") {
    switch (s) {
      case "600X600":  return [14, 14];
      case "300X300":  return [22, 22];
      case "800X800":  return [11, 11];
      case "600X1200": return [14, 7];
      case "300X600":  return [22, 11];
      case "300X1200":
      case "200X1200": return [24, 4];
      case "300X450":  return [22, 15];
      default:         return [12, 12];
    }
  } else {
    switch (s) {
      case "600X600":  return [7, 6];
      case "300X300":  return [11, 10];
      case "800X800":  return [5.5, 5];
      case "600X1200": return [7, 3.5];
      case "300X600":  return [11, 5.5];
      case "300X1200":
      case "200X1200": return [12, 2];
      case "300X450":  return [11, 7.5];
      default:         return [6, 5];
    }
  }
}
...
export default function Preview3D() {
  ...
  const handleElsRef            = useRef<Record<Corner, HTMLDivElement | null>>({ tl: null, tr: null, bl: null, br: null });
  ...
  const [hasAreaPatch,       setHasAreaPatch]       = useState<Partial<Record<WallTarget, boolean>>>({});
  ...
  useEffect(() => {
    ...
    const onTile = (event: Event) => {
      ...
      if (detail?.tile) {
        ...
        selectedTileRef.current = detail.tile;
      }
      if (detail?.size !== undefined) {
        setSelectedTileSize(detail.size);
        selectedTileSizeRef.current = detail.size;
      }
    };
  ...
  // ── Toolbar actions ───────────────────────────────────────────────────────
  const [activModal, setActivModal] = useState<"save" | "share" | "mail" | null>(null);
  const [mailForm, setMailForm] = useState({ name: "", to: "", subject: "", message: "" });
  ...
  const handleSaveImage = () => {
    ...
    if (!canvas) return;
    // Same as vis_cvs.toDataURL_ in old JS — white background composite
  ...
  // Build design share link (same as createDesignShareLink in old JS)
  ...
  const handleSavePDF = async () => {
    ...
    // Capture room image — needs preserveDrawingBuffer: true on renderer
    const canvas = canvasRef.current;
    if (!canvas) return;
    const c = document.createElement("canvas");
    c.width = canvas.width; c.height = canvas.height;
    ...
    // Header — same as old JS saveDesignInfoPdf
    ...
    // Helper: load image via proxy → base64 (same as old JS loadImageForPdf)
      ...
      new Promise((resolve) => {
        ...
        img.onload = () => {
          try {
            ...
            resolve({ dataUrl: ic.toDataURL("image/jpeg", 0.95), w: ic.width, h: ic.height });
          } catch { resolve(null); }
        };
    ...
    // Draw each tile card (same structure as old JS drawProductCard)
    const allTiles = [
    ...
    for (const tile of allTiles) {
      if (y + cardH + 6 > 285) { pdf.addPage(); y = 16; }
      ...
      const productLink = tile.skuCode
        ? `${window.location.origin}/product-details?sku=${encodeURIComponent(tile.skuCode)}`
        : "";
      ...
      // Section title
      pdf.setTextColor(18, 48, 47); pdf.setFontSize(13); pdf.setFont("helvetica", "bold");
      pdf.text(tile.section, leftX, y + 10);

      // Tile details
      pdf.setTextColor(34, 51, 59); pdf.setFont("helvetica", "normal"); pdf.setFontSize(10);
      ...
      // Tile thumbnail — proxied
    //   const tileImgUrl = tile.image ? `/api/tile-image?url=${encodeURIComponent(tile.image)}` : "";
    const tileImgUrl = tile.image ?? "";
      const tileImgData = await loadImg(tileImgUrl);
      if (tileImgData?.dataUrl) {
        // fitRect — same as old JS
        const r = Math.min(imgBox.w / tileImgData.w, imgBox.h / tileImgData.h);
        const fw = tileImgData.w * r; const fh = tileImgData.h * r;
        const fx = imgBox.x + (imgBox.w - fw) / 2; const fy = imgBox.y + (imgBox.h - fh) / 2;
        pdf.addImage(tileImgData.dataUrl, "JPEG", fx, fy, fw, fh);
      } else {
        pdf.setFontSize(8); pdf.text("Image unavailable", imgBox.x + 3, imgBox.y + 18);
      }

      // Product link
      if (productLink) {
        try {
          pdf.setTextColor(14, 88, 86); pdf.setFontSize(10);
          pdf.textWithLink("Open product page", leftX, y + 44, { url: productLink });
        } catch { pdf.text(`Product: ${productLink}`, leftX, y + 44); }
      }

      // QR code — small delay to avoid proxy rate limiting
      await new Promise(r => setTimeout(r, 300));
      const qrDataUrl = await fetchQr(productLink);
      if (qrDataUrl) {
        const fmt = qrDataUrl.startsWith("data:image/png") ? "PNG" : "JPEG";
        pdf.addImage(qrDataUrl, fmt, qrBox.x + 1, qrBox.y + 1, qrBox.w - 2, qrBox.h - 2);
      } else {
        pdf.setFontSize(8); pdf.text("QR", qrBox.x + 14, qrBox.y + 20);
      }
  ...
  const handleShareSocial = (service: string) => {
    // Same as old JS share-link click handler
    ...
    else if (service === "twitter") url = `https://twitter.com/intent/tweet?url=${encodedUrl}`;
    else if (service === "google")  url = `https://plus.google.com/share?url=${encodedUrl}`;
    else if (service === "whatsapp") url = `https://wa.me/?text=${encodeURIComponent("Check out my tile design! " + designLink)}`;
    if (url) window.open(url, "sharer", "width=626,height=436");
    setActivModal(null);
  };

  const handleMailSend = async () => {
    if (!mailForm.to || !mailForm.subject) { alert("Please fill in recipient email and subject."); return; }
    setMailSending(true);
    try {
      // Capture canvas as image (same as vis_cvs.toDataURL_ in old JS)
      const canvas = canvasRef.current;
      const imgUrl = canvas ? (() => {
        const c = document.createElement("canvas");
        c.width = canvas.width; c.height = canvas.height;
      ...
      const sceneKey   = localStorage.getItem("selected_3d_sub_scene") || "living_room";

      // Build tiles payload from applied tiles (matches old JS tiles structure)
      const tilesPayload: Record<string, any> = {};
      appliedWallTiles.forEach((t, i) => { tilesPayload[`wall_${i}`] = { name: t.name, image: t.image, size: t.size, skuCode: t.skuCode }; });
      appliedFloorTiles.forEach((t, i) => { tilesPayload[`floor_${i}`] = { name: t.name, image: t.image, size: t.size, skuCode: t.skuCode }; });
      ...
      if (res.ok) {
        setMailSent(true);
        setTimeout(() => { setMailSent(false); setActivModal(null); setMailForm({ name: "", to: "", subject: "", message: "" }); }, 2000);
      } else {
        // Fallback to mailto: (same as old JS fallbackToMailClient)
        const body = [`Name: ${mailForm.name}`, `Room: ${sceneKey}`, "", mailForm.message, "", `Design URL: ${designLink}`].join("\n");
        window.location.href = `mailto:${encodeURIComponent(mailForm.to)}?subject=${encodeURIComponent(mailForm.subject)}&body=${encodeURIComponent(body)}`;
        setActivModal(null);
      }
    } catch {
      // Fallback to mailto: on network error
      const body = [`Name: ${mailForm.name}`, "", mailForm.message, "", `Design URL: ${window.location.href}`].join("\n");
      window.location.href = `mailto:${encodeURIComponent(mailForm.to)}?subject=${encodeURIComponent(mailForm.subject)}&body=${encodeURIComponent(body)}`;
      setActivModal(null);
    } finally {
  ...
  const handlePrint = () => {
    // Same as old JS .print-btn click — open new window with canvas image
    const canvas = canvasRef.current;
    ...
    const WinPrint = window.open("", "", `width=${canvas.width},height=${canvas.height},toolbar=0,scrollbars=0,status=0`);
    if (!WinPrint) { window.print(); return; }
    WinPrint.document.write(content);
    WinPrint.document.close();
    WinPrint.focus();
    setTimeout(() => { WinPrint.print(); }, 1000);
    setTimeout(() => { WinPrint.close(); }, 2000);
  };
  const handleSelectRoom = () => {
    const space = (localStorage.getItem("selected_space_type") || "living").toLowerCase();
    localStorage.removeItem("force_3d_mode");
    localStorage.removeItem("selected_3d_sub_scene");
    window.dispatchEvent(new Event("storage"));
    window.dispatchEvent(new CustomEvent("force3DMode"));
  ...
  const getTileKey = (tile: AppliedTile) => {
    ...
    if (id) return id;
    // fallback: name+size to catch duplicates when skuCode is missing
  ...
  const addUniqueTile = (list: AppliedTile[], tile: AppliedTile | null) => {
    if (!tile) return list;
    const key = getTileKey(tile);
    if (key && list.some((t) => getTileKey(t) === key)) return list;
    return [tile, ...list];
  };

  const registerAppliedTile = useCallback((surface: "wall" | "floor") => {
    const tile = buildAppliedTile(selectedTileRef.current, selectedTileSizeRef.current);
    if (!tile) return;
    // Replace the entire list — each surface shows only the currently applied tile
    if (surface === "floor") {
      setAppliedFloorTiles([tile]);
    } else {
      setAppliedWallTiles((prev) => {
        // For walls, keep unique tiles per wall but replace same-key entries
        const key = getTileKey(tile);
...
const getTileImageSrc = (src?: string) => src ?? "";

  // ── Texture builder ───────────────────────────────────────────────────────
  const buildTexture = useCallback((base: THREE.Texture, rx: number, ry: number): THREE.Texture => {
    const img = base.image as HTMLImageElement;
    const cv  = document.createElement("canvas");
    cv.width  = img.width; cv.height = img.height;
    const ctx = cv.getContext("2d")!;
    ctx.drawImage(img, 0, 0, cv.width, cv.height);
    ctx.strokeStyle = "#4f4942"; ctx.lineWidth = 3;
    ctx.strokeRect(1.5, 1.5, cv.width - 3, cv.height - 3);
    ctx.strokeStyle = "#e6e1da"; ctx.lineWidth = 1;
  ...
  const loadAndApply = useCallback((applyFn: (base: THREE.Texture) => void) => {
    const tile = selectedTileRef.current;        // ← always the latest tile
    if (!tile?.image) { setErrorMessage("No tile selected."); return; }
    setIsApplying(true); setErrorMessage(null);
    ...
    loader.load(
      ...
      () => {},
      () => { setErrorMessage("Failed to load tile texture."); setIsApplying(false); }
    );
  }, []); // ← empty deps: ref is always current, no stale closure possible

  const applyFloor = useCallback(() => {
    if (!floorMatRef.current) return;
    const [rx, ry] = getRepeat(selectedTileSizeRef.current, "floor"); // ← ref
    registerAppliedTile("floor");
    loadAndApply((base) => applyTexToMat(floorMatRef.current!, buildTexture(base, rx, ry)));
  }, [loadAndApply, applyTexToMat, buildTexture, registerAppliedTile]);

  const applyWall = useCallback((surfaceName: string) => {
    const idx = surfaceName === SURFACE_NAMES.wallBack  ? 0
              : surfaceName === SURFACE_NAMES.wallLeft  ? 1
              : surfaceName === SURFACE_NAMES.wallRight ? 2 : -1;
    if (idx < 0 || !wallMatsRef.current[idx]) return;
    const [rx, ry] = getRepeat(selectedTileSizeRef.current, "wall"); // ← ref
    registerAppliedTile("wall");
    loadAndApply((base) => applyTexToMat(wallMatsRef.current[idx], buildTexture(base, rx, ry)));
  }, [loadAndApply, applyTexToMat, buildTexture, registerAppliedTile]);
  ...
  const applyAreaTileOnWall = useCallback((
    ...
    loadAndApply((base) => {
      ...
      if (areaMeshesRef.current[wall]) {
        ...
        (areaMeshesRef.current[wall]!.material as THREE.MeshStandardMaterial).dispose();
      ...
      const mat  = new THREE.MeshStandardMaterial({ map: tex, roughness: 0.6, metalness: 0.05, transparent: false, opacity: 1.0 });
  ...
  const clearAreaPatch = useCallback((wall: WallTarget) => {
    ...
    if (mesh) {
      ...
      delete areaMeshesRef.current[wall];
      setHasAreaPatch(prev => { const n = { ...prev }; delete n[wall]; return n; });
    }
  }, []);
  ...
  const clearPreviewBox = useCallback(() => {
    if (previewBoxRef.current && sceneRef.current) {
      ...
      if (outline instanceof THREE.LineSegments) {
        outline.geometry.dispose(); (outline.material as THREE.Material).dispose();
      }
  ...
  const showFurnitureOutline = useCallback((obj: THREE.Group) => {
    ...
    obj.traverse((c: THREE.Object3D) => {
      ...
      const line  = new THREE.LineSegments(edges, new THREE.LineBasicMaterial({ color: 0xf0b43c, linewidth: 2 }));
  ...
  const hideHandles = useCallback(() => {
    (Object.values(handleElsRef.current) as (HTMLDivElement | null)[])
  ...
  const refreshHandles = useCallback(() => {
    ...
    obj.traverse((child: THREE.Object3D) => {
      ...
      for (let i = 0; i < pos.count; i++) {
        ...
        mesh.localToWorld(v); v.project(camera);
    ...
    if (!isFinite(minX)) { hideHandles(); return; }
    const pad = 5;
    minX -= pad; minY -= pad; maxX += pad; maxY += pad;
    const corners: Record<Corner, [number, number]> = {
      tl: [minX, minY], tr: [maxX, minY], bl: [minX, maxY], br: [maxX, maxY],
    };
    (Object.entries(corners) as [Corner, [number, number]][]).forEach(([key, [sx, sy]]) => {
      const el = els[key];
      if (!el) return;
  ...
  const selectFurniture = useCallback((obj: THREE.Group) => {
    ...
    showFurnitureOutline(obj); refreshHandles();
  ...
  const deselectFurniture = useCallback(() => {
    ...
    clearFurnitureOutline(); hideHandles(); setFurnitureLabel(null);
    if (controlsRef.current) controlsRef.current.enabled = modeRef.current === "orbit";
  ...
  const hitMovable = useCallback((e: React.MouseEvent): THREE.Group | null => {
    ...
    movablesRef.current.forEach(g => g.traverse(c => { if ((c as THREE.Mesh).isMesh) meshes.push(c as THREE.Mesh); }));
    const hits = raycasterRef.current.intersectObjects(meshes, false);
    if (!hits.length) return null;
    let cur: THREE.Object3D | null = hits[0].object;
    while (cur) { if (cur.userData.movable) return cur as THREE.Group; cur = cur.parent; }
    return null;
  }, []);

  const isDescendantOfSelected = (node: THREE.Object3D): boolean => {
    let cur: THREE.Object3D | null = node;
    while (cur) { if (cur === selectedFurniture.current) return true; cur = cur.parent; }
    return false;
  };
  ...
  const getFloorPoint = useCallback((e: React.MouseEvent, atY = 0): THREE.Vector3 | null => {
    ...
    raycasterRef.current.setFromCamera(ndc, cameraRef.current);
    // Use the object's own Y level so dragging tracks the actual surface
  ...
  useEffect(() => {
    ...
    controls.minDistance = 3; controls.maxDistance = 11;
    controls.minPolarAngle = 0.2; controls.maxPolarAngle = Math.PI / 2.1;
    ...
    dir.position.set(0, 12, 0); dir.castShadow = true;
    ...
    floorMatRef.current = null; wallMatsRef.current = [];
    const sceneKey = (localStorage.getItem("selected_3d_sub_scene") || "living_room").toLowerCase();
    const builder  = sceneBuilders[sceneKey] || buildLivingRoomScene;
    const refs = builder({
      scene,
      onFloorMaterialReady: (mat) => { floorMatRef.current = mat; },
      onWallMaterialsReady: (mats) => { wallMatsRef.current = mats; },
    });

    const allRefs = refs as AnyRoomRefs | undefined;
    bedroomRefsRef.current    = null; kitchenRefsRef.current    = null;
    bathroomRefsRef.current   = null; livingRoomRefsRef.current = null;
    ...
    if (typeof ResizeObserver !== "undefined" && canvas.parentElement) {
      ro = new ResizeObserver(onResize); ro.observe(canvas.parentElement);
    }

    const ROOM_X = 8.5, ROOM_Z_MIN = -6.5, ROOM_Z_MAX = 6.5, ROOM_Y_MIN = 0.5, ROOM_Y_MAX = 5.5;
    let frameId: number;
    const animate = () => {
      frameId = requestAnimationFrame(animate);
      controls.update();
      camera.position.x = Math.max(-ROOM_X,     Math.min(ROOM_X,     camera.position.x));
      camera.position.z = Math.max(ROOM_Z_MIN,  Math.min(ROOM_Z_MAX, camera.position.z));
      camera.position.y = Math.max(ROOM_Y_MIN,  Math.min(ROOM_Y_MAX, camera.position.y));
    ...
    return () => {
      ...
      ro?.disconnect(); cancelAnimationFrame(frameId);
      renderer.dispose(); controls.dispose();
      sceneRef.current = rendererRef.current = controlsRef.current = cameraRef.current = null;
    };
  }, [refreshHandles]);

  // ── Attach scale-handle pointerdown ──────────────────────────────────────
  useEffect(() => {
    (Object.entries(handleElsRef.current) as [Corner, HTMLDivElement | null][]).forEach(([corner, el]) => {
      if (!el) return;
      el.addEventListener("pointerdown", (e: PointerEvent) => {
        const sel = selectedFurniture.current;
        if (!sel) return;
        handleDragRef.current = { corner, scaleStart: sel.scale.clone(), mouseX: e.clientX, mouseY: e.clientY };
  ...
  const toggleLamp = useCallback((lampName: string) => {
    const refs = bedroomRefsRef.current; if (!refs) return;
    ...
    light.intensity = turningOn ? 12 : 0; shadeMat.emissiveIntensity = turningOn ? 1.0 : 0.0; shadeMat.needsUpdate = true;
    const bothOff = refs.leftLampLight.intensity === 0 && refs.rightLampLight.intensity === 0;
    refs.ambientLight.intensity = bothOff ? 0.25 : 0.5; refs.hemisphereLight.intensity = bothOff ? 0.3 : 0.6;
  }, []);

  const toggleTubeLight = useCallback((tubeName: string) => {
    const refs = kitchenRefsRef.current ?? bathroomRefsRef.current; if (!refs) return;
    const isFirst   = tubeName === KIT_TUBE_NAMES.tube1 || tubeName === BATH_TUBE_NAMES.tube1;
    const light     = isFirst ? refs.tubeLight1 : refs.tubeLight2;
    const tubeMat   = isFirst ? refs.tubeMat1   : refs.tubeMat2;
    const turningOn = light.intensity === 0;
    light.intensity = turningOn ? 18 : 0; tubeMat.emissiveIntensity = turningOn ? 2.5 : 0.0; tubeMat.needsUpdate = true;
    const bothOff = refs.tubeLight1.intensity === 0 && refs.tubeLight2.intensity === 0;
    refs.ambientLight.intensity = bothOff ? 0.15 : 0.35; refs.hemisphereLight.intensity = bothOff ? 0.2 : 0.45;
  }, []);

  const toggleLivingLight = useCallback((lightName: string) => {
    const refs = livingRoomRefsRef.current; if (!refs) return;
    if (lightName === LIVING_LIGHT_NAMES.tubeLight1 || lightName === LIVING_LIGHT_NAMES.tubeLight2) {
      const isFirst   = lightName === LIVING_LIGHT_NAMES.tubeLight1;
      const light     = isFirst ? refs.tubeLight1 : refs.tubeLight2;
      const mat       = isFirst ? refs.tubeMat1   : refs.tubeMat2;
      const turningOn = light.intensity === 0;
      light.intensity = turningOn ? 22 : 0; mat.emissiveIntensity = turningOn ? 2.5 : 0.0; mat.needsUpdate = true;
    } else if (lightName === LIVING_LIGHT_NAMES.floorShade) {
      const turningOn = refs.floorLampLight.intensity === 0;
      refs.floorLampLight.intensity = turningOn ? 12 : 0; refs.floorShadeMat.emissiveIntensity = turningOn ? 1.0 : 0.0; refs.floorShadeMat.needsUpdate = true;
    } else if (lightName === LIVING_LIGHT_NAMES.pendantShade) {
      const turningOn = refs.pendantLight.intensity === 0;
      refs.pendantLight.intensity = turningOn ? 15 : 0; refs.pendantShadeMat.emissiveIntensity = turningOn ? 0.6 : 0.0; refs.pendantShadeMat.needsUpdate = true;
    }
    const allOff = refs.floorLampLight.intensity === 0 && refs.pendantLight.intensity === 0 && refs.tubeLight1.intensity === 0 && refs.tubeLight2.intensity === 0;
    refs.ambientLight.intensity = allOff ? 0.1 : 0.5; refs.hemisphereLight.intensity = allOff ? 0.2 : 0.7;
  }, []);
  ...
  const raycastAnyWall = (ndc: THREE.Vector2): THREE.Object3D | null => {
    ...
    const walls = ([SURFACE_NAMES.wallBack, SURFACE_NAMES.wallLeft, SURFACE_NAMES.wallRight] as string[])
      .map(n => sceneRef.current!.getObjectByName(n)).filter(Boolean) as THREE.Object3D[];
  ...
  const findLightName = (obj: THREE.Object3D, names: string[]): string | null => {
    ...
    while (cur) { if (names.includes(cur.name)) return cur.name; cur = cur.parent; }
    return null;
  };
  ...
  const raycastLamp = (ndc: THREE.Vector2): string | null => {
    ...
    if (livingRoomRefsRef.current) targetNames.push(LIVING_LIGHT_NAMES.floorShade, LIVING_LIGHT_NAMES.pendantShade, LIVING_LIGHT_NAMES.tubeLight1, LIVING_LIGHT_NAMES.tubeLight2);
    if (targetNames.length === 0) return null;
    const targets = targetNames.map(n => sceneRef.current!.getObjectByName(n)).filter(Boolean) as THREE.Object3D[];
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    if (handleDragRef.current) {
      const hd = handleDragRef.current; const sel = selectedFurniture.current; if (!sel) return;
      const dx = e.clientX - hd.mouseX; const dy = e.clientY - hd.mouseY;
      const sgnX = (hd.corner === "tr" || hd.corner === "br") ?  1 : -1;
      const sgnY = (hd.corner === "tl" || hd.corner === "tr") ? -1 :  1;
      sel.scale.x = Math.max(0.2, Math.min(3.5, hd.scaleStart.x * (1 + sgnX * dx * 0.004)));
      sel.scale.z = Math.max(0.2, Math.min(3.5, hd.scaleStart.z * (1 + sgnX * dx * 0.004)));
      sel.scale.y = Math.max(0.2, Math.min(3.5, hd.scaleStart.y * (1 + sgnY * dy * 0.004)));
      syncFurnitureOutline(); return;
    }
    if (furnitureDragging.current && selectedFurniture.current) {
      const fp = getFloorPoint(e); const start = furnitureDragFloorStart.current; const objStart = furnitureDragObjStart.current;
      if (!fp || !start || !objStart) return;
      const sceneKey = (localStorage.getItem("selected_3d_sub_scene") || "living_room").toLowerCase();
      // Per-room bounds based on actual wall positions — keeps furniture always visible
      const bounds = sceneKey.includes("bathroom")
        ...
        : sceneKey.includes("bedroom")
          ...
          : sceneKey.includes("kitchen")
            ? { x: 7.2, z: 6.2 }
            : { x: 9.2, z: 7.2 }; // living room
      const newX = objStart.x + fp.x - start.x;
      const newZ = objStart.z + fp.z - start.z;
      selectedFurniture.current.position.x = Math.max(-bounds.x, Math.min(bounds.x, newX));
      selectedFurniture.current.position.z = Math.max(-bounds.z, Math.min(bounds.z, newZ));
      syncFurnitureOutline(); return;
    }
    ...
    if (m === "draw-area-dragging") {
      const wall = areaWallRef.current; const p1 = drawStartHitRef.current;
      if (!wall || !p1 || !sceneRef.current) return;
      const pt = raycastWallHit(getNDC(e), wall); if (!pt) return;
      const isBack = wall === SURFACE_NAMES.wallBack;
      const cx = (p1.x + pt.x) / 2, cy = (p1.y + pt.y) / 2, cz = (p1.z + pt.z) / 2;
      ...
      if (!previewBoxRef.current) {
        ...
        const mat  = new THREE.MeshBasicMaterial({ color: 0xF59E0B, transparent: true, opacity: 0.12, side: THREE.DoubleSide, depthWrite: false });
        const mesh = new THREE.Mesh(geo, mat); mesh.renderOrder = 2;
        sceneRef.current.add(mesh); previewBoxRef.current = mesh;
        const edgeGeo = new THREE.EdgesGeometry(new THREE.PlaneGeometry(1, 1));
        const outline = new THREE.LineSegments(edgeGeo, new THREE.LineBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.85 }));
        outline.renderOrder = 3; outline.name = "preview_outline"; mesh.add(outline);
      }
      const OFFSET = 0.014;
      previewBoxRef.current.scale.set(w, h, 1);
      if (isBack) { previewBoxRef.current.rotation.set(0, 0, 0); previewBoxRef.current.position.set(cx, cy, wallPos.z + OFFSET); }
      else if (wall === SURFACE_NAMES.wallLeft) { previewBoxRef.current.rotation.set(0, Math.PI / 2, 0); previewBoxRef.current.position.set(wallPos.x + OFFSET, cy, cz); }
      else { previewBoxRef.current.rotation.set(0, -Math.PI / 2, 0); previewBoxRef.current.position.set(wallPos.x - OFFSET, cy, cz); }
      return;
    }
    if (m === "orbit" && canvasRef.current) canvasRef.current.style.cursor = "grab";
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    if (m === "orbit") {
      ...
      if (hit) {
        // Single click — select immediately and start dragging
    ...
    if (m === "draw-area") {
      const hit = raycastAnyWall(getNDC(e)); if (!hit) return;
      const wall = hit.name as WallTarget; const pt = raycastWallHit(getNDC(e), wall); if (!pt) return;
      areaWallRef.current = wall; drawStartHitRef.current = pt; setAreaWallChosen(wall);
      modeRef.current = "draw-area-dragging"; setMode("draw-area-dragging"); e.stopPropagation();
    }
  }, [hitMovable, getFloorPoint, deselectFurniture, selectFurniture]);

  const handleMouseUp = useCallback((e: React.MouseEvent) => {
    const m = modeRef.current;
    if (furnitureDragging.current) { furnitureDragging.current = false; furnitureDragFloorStart.current = null; return; }
    if (handleDragRef.current) { handleDragRef.current = null; return; }
    if (m === "pick-wall") {
      const hit = raycastAnyWall(getNDC(e));
      if (hit && selectedTileRef.current) {
        applyWall(hit.name); setMode("orbit"); setHoveredWall(null);
    ...
    if (m === "draw-area-dragging") {
      ...
      if (wall && p1) {
        ...
        if (pt) {
          ...
          if (hSize > 0.2 && vSize > 0.2) { setPendingAreaData({ wall, p1, p2: pt }); setAreaConfirmPending(true); return; }
        }
      }
      drawStartHitRef.current = null; modeRef.current = "draw-area"; setMode("draw-area");
    }
  ...
  const handleDoubleClick = useCallback((_e: React.MouseEvent) => {
    // Single click now handles selection — double click intentionally left empty
  }, []);
  ...
  const confirmAreaTile = useCallback(() => {
    ...
    setPendingAreaData(null); setAreaConfirmPending(false);
    areaWallRef.current = null; drawStartHitRef.current = null;
    setAreaWallChosen(null); setMode("orbit"); modeRef.current = "orbit";
  }, [pendingAreaData, applyAreaTileOnWall]);

  const cancelArea = useCallback(() => {
    clearPreviewBox(); setPendingAreaData(null); setAreaConfirmPending(false);
    areaWallRef.current = null; drawStartHitRef.current = null;
    setAreaWallChosen(null); setMode("orbit"); modeRef.current = "orbit"; setHoveredWall(null);
  }, [clearPreviewBox]);
  ...
  const handleEsc = useCallback(() => {
    if (activModal) { setActivModal(null); return; }
    if (isProductInfoOpen) { setIsProductInfoOpen(false); return; }
    if (selectedFurniture.current) { deselectFurniture(); return; }
    if (mode !== "orbit") {
      clearPreviewBox(); setMode("orbit"); modeRef.current = "orbit";
      setHoveredWall(null); setAreaWallChosen(null); setAreaConfirmPending(false); setPendingAreaData(null);
      areaWallRef.current = null; drawStartHitRef.current = null;
    }
  }, [mode, clearPreviewBox, deselectFurniture, isProductInfoOpen, activModal]);

  const resizeRenderer = useCallback(() => {
    const camera = cameraRef.current; const renderer = rendererRef.current; const canvas = canvasRef.current;
  ...
  useEffect(() => {
    const onChange = () => { setIsFullscreen(!!document.fullscreenElement); setTimeout(resizeRenderer, 50); };
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full bg-[#F9F7F2] overflow-hidden">
      <canvas
        ref={canvasRef}
        style={{ display: "block", position: "absolute", inset: 0, width: "100%", height: "100%", cursor: mode === "orbit" ? "grab" : "crosshair" }}
      ...
      <div className="absolute top-6 right-6 z-50 flex items-center gap-3">
        <button onClick={handleSelectRoom} className="px-4 py-2 rounded-full bg-white/90 border border-slate-200 text-[12px] font-semibold shadow-sm">Select Room</button>
        <button onClick={handleProductInfo} className="px-4 py-2 rounded-full bg-white/90 border border-slate-200 text-[12px] font-semibold shadow-sm">Product Info</button>
      </div>

      {/* Right toolbar */}
      <div className="absolute top-1/2 right-5 -translate-y-1/2 z-50 flex flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white/95 shadow-lg">
        <button onClick={() => setActivModal("save")} className="p-3 border-b border-slate-200 hover:bg-slate-50" title="Save Design"><Save size={18} /></button>
        <button onClick={handlePrint} className="p-3 border-b border-slate-200 hover:bg-slate-50" title="Print"><Printer size={18} /></button>
        <button onClick={() => setActivModal("mail")} className="p-3 border-b border-slate-200 hover:bg-slate-50" title="Email"><Mail size={18} /></button>
        <button onClick={() => setActivModal("share")} className="p-3 border-b border-slate-200 hover:bg-slate-50" title="Share"><Share2 size={18} /></button>
        <button onClick={toggleFullscreen} className="p-3 hover:bg-slate-50" title="Fullscreen">
      ...
      {selectedTile && (
        <div className="absolute top-24 right-6 z-40 bg-white/90 backdrop-blur-sm p-3 rounded-xl shadow-lg border border-slate-200 max-w-[160px]">
          ...
          {/* <img src={`${IMAGE_PROXY_URL}${encodeURIComponent(selectedTile.image)}`} alt={selectedTile.name} className="w-full h-auto rounded-lg object-cover" /> */}
          <img src={selectedTile.image} alt={selectedTile.name} className="w-full h-auto rounded-lg object-cover" /> 
      ...
      {hoveredWall && (mode === "pick-wall" || mode === "draw-area") && (
        <div className="absolute bottom-36 left-1/2 -translate-x-1/2 z-40 pointer-events-none">
          <div className={`text-white text-xs font-semibold px-4 py-2 rounded-full shadow-lg ${mode === "pick-wall" ? "bg-amber-500" : "bg-violet-500"}`}>
            {WALL_LABELS[hoveredWall] ?? "Wall"} — {mode === "pick-wall" ? "click to tile" : "click + drag to draw area"}
          </div>
        </div>
      )}
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50">
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 p-6 flex flex-col items-center gap-4 min-w-[280px]">
            ...
            <p className="text-slate-500 text-xs text-center">
              Selected area on <span className="font-semibold text-slate-700">{WALL_LABELS[pendingAreaData.wall]}</span>
            </p>
            <div className="flex gap-3 w-full">
              <button onClick={cancelArea} className="flex-1 py-2.5 rounded-xl border border-slate-200 text-slate-600 text-sm hover:bg-slate-50 transition-all">Cancel</button>
              <button onClick={confirmAreaTile} className="flex-1 py-2.5 rounded-xl bg-violet-500 text-white text-sm font-semibold hover:bg-violet-600 transition-all">Apply Tile</button>
            </div>
          </div>
        </div>
      )}

      {/* Furniture selection label */}
      ...
      {(["tl", "tr", "bl", "br"] as Corner[]).map(c => (
        <div
          key={c}
          ref={el => { handleElsRef.current[c] = el; }}
          ...
          style={{
            display: "none",
            cursor: c === "tl" ? "nw-resize" : c === "tr" ? "ne-resize" : c === "bl" ? "sw-resize" : "se-resize",
            touchAction: "none",
          }}
        />
      ))}
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-40 flex items-end gap-3">
        <div className="flex flex-col items-center gap-1.5">
          ...
          <button
            ...
            <span className="relative flex items-center gap-2"><Layers size={13} /> Entire Floor</span>
          </button>
        </div>
        ...
        <div className="flex flex-col items-center gap-1.5">
          ...
          <div className="flex gap-2">
            <button
              disabled={!tileReady}
              onClick={() => {
                if (mode === "draw-area" || mode === "draw-area-dragging") { cancelArea(); }
                else { setMode("draw-area"); modeRef.current = "draw-area"; }
              }}
            ...
            {(Object.keys(hasAreaPatch) as WallTarget[]).filter(w => hasAreaPatch[w]).map(w => (
              <button key={w} onClick={() => clearAreaPatch(w)}
      ...
      {/* Product Info Modal */}
      {isProductInfoOpen && (
        <div className="absolute inset-0 z-[80] flex items-center justify-center bg-black/30 backdrop-blur-sm" onClick={() => setIsProductInfoOpen(false)}>
          <div className="w-[88vw] max-w-[900px] max-h-[80vh] bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
              ...
              <button onClick={() => setIsProductInfoOpen(false)} className="w-8 h-8 rounded-full border border-slate-200 flex items-center justify-center text-slate-600 hover:bg-slate-50"><X size={16} /></button>
            </div>
            <div className="px-6 pt-4">
              <div className="flex gap-2 border-b border-slate-200">
                {(["wall", "floor"] as const).map(tab => (
                  <button key={tab} onClick={() => setProductInfoTab(tab)}
                    className={`px-4 py-2 text-[11px] font-semibold rounded-t-lg border border-b-0 capitalize ${productInfoTab === tab ? "bg-slate-900 text-white border-slate-900" : "bg-white text-slate-600 border-slate-200"}`}>
                    {tab}
                  </button>
                ))}
              </div>
        ...
        <div className="absolute inset-0 z-[80] flex items-center justify-center bg-black/30 backdrop-blur-sm" onClick={() => setActivModal(null)}>
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 p-6 w-[320px]" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => setActivModal(null)} className="w-7 h-7 rounded-full border border-slate-200 flex items-center justify-center text-slate-500 hover:bg-slate-50"><X size={14} /></button>
            </div>
            <div className="flex flex-col gap-3">
              <button onClick={() => { handleSaveImage(); setActivModal(null); }}
              ...
              <button onClick={() => { handleSavePDF(); setActivModal(null); }}
                className="w-full py-3 rounded-xl border border-slate-200 text-slate-700 text-[11px] font-bold uppercase tracking-widest hover:bg-slate-50 transition-all flex items-center justify-center gap-2">
                <Printer size={14} /> Save as PDF
              </button>
              <button onClick={() => { handleSaveLink(); setActivModal(null); }}
                className="w-full py-3 rounded-xl border border-slate-200 text-slate-700 text-[11px] font-bold uppercase tracking-widest hover:bg-slate-50 transition-all flex items-center justify-center gap-2">
      ...
      {activModal === "share" && (
        <div className="absolute inset-0 z-[80] flex items-center justify-center bg-black/30 backdrop-blur-sm" onClick={() => setActivModal(null)}>
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 p-6 w-[320px]" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex flex-col gap-3">
              <button onClick={() => handleShareSocial("facebook")}
              ...
              <button onClick={() => handleShareSocial("twitter")}
                className="w-full py-3 rounded-xl text-white text-[11px] font-bold uppercase tracking-widest transition-all flex items-center justify-center gap-2"
                style={{ backgroundColor: "#1DA1F2" }}>
                Twitter / X
              </button>
              <button onClick={() => handleShareSocial("google")}
                className="w-full py-3 rounded-xl text-white text-[11px] font-bold uppercase tracking-widest transition-all flex items-center justify-center gap-2"
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 p-6 w-[380px]" onClick={e => e.stopPropagation()}>
            ...
            {mailSent ? (
              ...
              <div className="flex flex-col gap-3">
                ...
                />
                <input
                  type="email" placeholder="Recipient's Email Address"
                  value={mailForm.to}
