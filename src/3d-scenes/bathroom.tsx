...
export function buildBathroomScene({
  ...
  const movables: THREE.Group[] = [];
 const markMovable = (obj: THREE.Group, name: string, label: string, surface: "floor" | "wall" = "floor") => {
  obj.userData.movable = true;
  obj.userData.label   = label;
  obj.userData.surface = surface;
  obj.name = name;
  movables.push(obj);
};
  ...
  // FIX: tube light point lights — symmetric left/right matching kitchen pattern
  // Kitchen uses x = -4.2 and +4.2 on a 16-wide room
  // Bathroom is 12-wide so scale proportionally: -3.0 and +3.0
  const tubeLightPL1 = new THREE.PointLight(0xfff8f0, 20, 28, 1.5);
  tubeLightPL1.position.set(-3.0, 4.92, -1.0);
  tubeLightPL1.castShadow = true;
  scene.add(tubeLightPL1);

  const tubeLightPL2 = new THREE.PointLight(0xfff8f0, 20, 28, 1.5);
  tubeLightPL2.position.set(3.0, 4.92, -1.0);
  tubeLightPL2.castShadow = true;
  scene.add(tubeLightPL2);
  ...
  const tubeMat1 = new THREE.MeshStandardMaterial({
    color: 0xffffff, roughness: 0.15, metalness: 0.1,
    emissive: new THREE.Color(0xfff8e8), emissiveIntensity: 3.0,
    transparent: true, opacity: 0.92,
  });
  const tubeMat2 = new THREE.MeshStandardMaterial({
    color: 0xffffff, roughness: 0.15, metalness: 0.1,
  ...
  const backWall = new THREE.Mesh(new THREE.PlaneGeometry(12, 5), backWallMat);
  backWall.position.set(0, 2.5, -4.5);
  backWall.receiveShadow = true; backWall.name = SURFACE_NAMES.wallBack; scene.add(backWall);
  ...
  ceiling.rotation.x = Math.PI / 2; ceiling.position.y = 5;
  ceiling.receiveShadow = true; scene.add(ceiling);

  const baseboard = new THREE.Mesh(new THREE.BoxGeometry(12, 0.15, 0.08),
    new THREE.MeshStandardMaterial({ color: 0xf5f5f5, roughness: 0.6 }));
  baseboard.position.set(0, 0.075, -4.46); scene.add(baseboard);

  // ==============================================
  // Tube Lights — ceiling mounted
  // FIX: x = -3.0 (left) and +3.0 (right) symmetric, matching kitchen ratio
  // FIX: proxy spheres at y=1.8 (eye/reach level), radius=1.2 for easy clicking
  // ==============================================
  function createTubeLight(
    x: number,
    z: number,
    ...
    bracket.position.y = -0.025; g.add(bracket);
    ...
    housing.position.y = -0.09; g.add(housing);
    ...
    for (const [cx, cz] of capPositions) {
      const cap = new THREE.Mesh(new THREE.BoxGeometry(0.16, 0.09, 0.16),
        new THREE.MeshStandardMaterial({ color: 0xb0b0b0, roughness: 0.4, metalness: 0.5 }));
      cap.position.set(cx, -0.09, cz); g.add(cap);
    }

    g.position.set(x, 4.98, z);
    scene.add(g);

    // FIX: proxy at eye level y=1.8, radius=1.2 for easy clicking
    const proxy = new THREE.Mesh(
      new THREE.SphereGeometry(1.2, 8, 8),
      new THREE.MeshBasicMaterial({ transparent: true, opacity: 0, depthWrite: false })
    );
    proxy.name = tubeName;
    proxy.position.set(x, 1.8, z);
    scene.add(proxy);
  }

  // FIX: -3.0 left and +3.0 right — symmetric, matching kitchen pattern
  createTubeLight(-3.0, -1.0, tubeMat1, TUBE_LIGHT_NAMES.tube1, 'x');
  createTubeLight( 3.0, -1.0, tubeMat2, TUBE_LIGHT_NAMES.tube2, 'x');
  ...
  const vanityBase = new THREE.Mesh(new THREE.BoxGeometry(2.6, 0.95, 1.1), vanityMat);
  vanityBase.position.set(0, 0.575, 0); vanityBase.castShadow = true; vanityGroup.add(vanityBase);

  const countertop = new THREE.Mesh(new THREE.BoxGeometry(2.7, 0.08, 1.15),
    new THREE.MeshStandardMaterial({ color: 0x2a2a2a, roughness: 0.2, metalness: 0.4 }));
  countertop.position.set(0, 1.09, 0); countertop.castShadow = true; vanityGroup.add(countertop);

  const sinkOuter = new THREE.Mesh(new THREE.CylinderGeometry(0.34, 0.31, 0.22, 32), porcelainMat);
  sinkOuter.position.set(0, 1.25, 0); sinkOuter.castShadow = true; vanityGroup.add(sinkOuter);

  const sinkInner = new THREE.Mesh(new THREE.CylinderGeometry(0.27, 0.24, 0.2, 32), porcelainMat);
  sinkInner.position.set(0, 1.28, 0); vanityGroup.add(sinkInner);

  const drain = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.03, 0.02, 16), chromeMat);
  drain.position.set(0, 1.16, 0); vanityGroup.add(drain);

  const faucetBase = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.05, 0.08, 16), chromeMat);
  faucetBase.position.set(0, 1.37, 0.3); vanityGroup.add(faucetBase);

  const faucetNeck = new THREE.Mesh(new THREE.CylinderGeometry(0.025, 0.025, 0.35, 12), chromeMat);
  faucetNeck.position.set(0, 1.6, 0.3); faucetNeck.castShadow = true; vanityGroup.add(faucetNeck);

  const faucetSpout = new THREE.Mesh(new THREE.CylinderGeometry(0.025, 0.02, 0.22, 12), chromeMat);
  faucetSpout.position.set(0, 1.7, 0.1); faucetSpout.rotation.x = Math.PI / 3; vanityGroup.add(faucetSpout);

  for (let i = 0; i < 2; i++) {
    const drawer = new THREE.Mesh(new THREE.BoxGeometry(1.1, 0.38, 0.05),
      new THREE.MeshStandardMaterial({ color: 0x5a3a1a, roughness: 0.7 }));
    drawer.position.set(i === 0 ? -0.6 : 0.6, 0.55, 0.54); vanityGroup.add(drawer);
    const dhandle = new THREE.Mesh(new THREE.CylinderGeometry(0.015, 0.015, 0.12, 12), chromeMat);
    dhandle.rotation.z = Math.PI / 2;
    dhandle.position.set(i === 0 ? -0.6 : 0.6, 0.55, 0.57); vanityGroup.add(dhandle);
  }

  vanityGroup.position.set(-4.4, 0, -3.1);
  markMovable(vanityGroup, 'furniture_vanity', 'Vanity & Sink','floor');
  scene.add(vanityGroup);
  ...
  const mirrorFrame = new THREE.Mesh(new THREE.BoxGeometry(2.8, 2.0, 0.06),
    new THREE.MeshStandardMaterial({ color: 0x2a2a2a, roughness: 0.3, metalness: 0.6 }));
  mirrorFrame.castShadow = true; mirrorGroup.add(mirrorFrame);

  const mirror = new THREE.Mesh(new THREE.PlaneGeometry(2.6, 1.8),
    new THREE.MeshStandardMaterial({ color: 0xe8f0f8, metalness: 1.0, roughness: 0.02, envMapIntensity: 2.5 }));
  mirror.position.set(0, 0, 0.06); mirrorGroup.add(mirror);

  const lightBar = new THREE.Mesh(new THREE.BoxGeometry(2.6, 0.08, 0.12),
    new THREE.MeshStandardMaterial({
      color: 0xf5f5f5, roughness: 0.2, metalness: 0.8,
      emissive: new THREE.Color(0xffffff), emissiveIntensity: 0.4,
    }));
  lightBar.position.set(0, 1.12, 0.06); mirrorGroup.add(lightBar);

  mirrorGroup.position.set(-4.4, 2.5, -4.22);
  markMovable(mirrorGroup, 'furniture_mirror', 'Mirror','wall');
  scene.add(mirrorGroup);
  ...
  const towelBar = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.03, 1.1, 16), chromeMat);
  towelBar.rotation.z = Math.PI / 2; towelBar.castShadow = true; towelGroup.add(towelBar);

  for (const tx of [-0.55, 0.55]) {
    const mount = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.06, 0.08, 16), chromeMat);
    mount.position.set(tx, 0, -0.02); towelGroup.add(mount);
  }

  const towel = new THREE.Mesh(new THREE.BoxGeometry(0.9, 0.55, 0.04),
    new THREE.MeshStandardMaterial({ color: 0xe8d8c8, roughness: 0.95 }));
  towel.position.set(0, -0.25, -0.02); towelGroup.add(towel);

  towelGroup.position.set(-2.2, 1.6, -4.3);
  markMovable(towelGroup, 'furniture_towel_holder', 'Towel Holder','wall');
  scene.add(towelGroup);
  ...
  const geyserBody = new THREE.Mesh(new THREE.BoxGeometry(1.1, 1.4, 0.45),
    new THREE.MeshStandardMaterial({ color: 0xf7f7f7, roughness: 0.35, metalness: 0.08 }));
  geyserGroup.add(geyserBody);

  const geyserFront = new THREE.Mesh(new THREE.SphereGeometry(0.65, 32, 24),
    new THREE.MeshStandardMaterial({ color: 0xfafafa, roughness: 0.25, metalness: 0.05 }));
  geyserFront.scale.set(1.2, 1.0, 0.25); geyserFront.position.set(0, 0.05, 0.25); geyserGroup.add(geyserFront);

  const geyserKnobRed = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.08, 0.12, 20),
    new THREE.MeshStandardMaterial({ color: 0xd23b3b, roughness: 0.3, metalness: 0.1 }));
  geyserKnobRed.rotation.x = Math.PI / 2; geyserKnobRed.position.set(-0.18, -0.65, 0.3); geyserGroup.add(geyserKnobRed);

  const geyserKnobBlue = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.08, 0.12, 20),
    new THREE.MeshStandardMaterial({ color: 0x2f6bdc, roughness: 0.3, metalness: 0.1 }));
  geyserKnobBlue.rotation.x = Math.PI / 2; geyserKnobBlue.position.set(0.18, -0.65, 0.3); geyserGroup.add(geyserKnobBlue);

  geyserGroup.position.set(0, 3.4, -4.2);
  markMovable(geyserGroup, 'furniture_geyser', 'Geyser','wall');
  scene.add(geyserGroup);
  ...
  const tapBody = new THREE.Mesh(new THREE.CylinderGeometry(0.12, 0.12, 0.6, 24), chromeMat);
  tapBody.rotation.z = Math.PI / 2; tapGroup.add(tapBody);

  const tapSpout = new THREE.Mesh(new THREE.BoxGeometry(0.6, 0.08, 0.16), chromeMat);
  tapSpout.position.set(0.35, -0.1, 0.1); tapGroup.add(tapSpout);

  const tapHandle = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.08, 0.12, 20), chromeMat);
  tapHandle.rotation.x = Math.PI / 2; tapHandle.position.set(-0.25, 0.15, 0.06); tapGroup.add(tapHandle);

  tapGroup.position.set(0, 1.2, -4.15);
  markMovable(tapGroup, 'furniture_tap', 'Wall Tap','wall');
  scene.add(tapGroup);
  ...
  const bucketBody = new THREE.Mesh(new THREE.CylinderGeometry(0.42, 0.34, 0.55, 32, 1, false),
    new THREE.MeshStandardMaterial({ color: 0xe8e8ea, roughness: 0.38, metalness: 0.15 }));
  bucketBody.position.y = 0.28; bucketBody.castShadow = true; bucketGroup.add(bucketBody);

  const bucketBottom = new THREE.Mesh(new THREE.CircleGeometry(0.34, 28),
    new THREE.MeshStandardMaterial({ color: 0xe8e8ea }));
  bucketBottom.rotation.x = -Math.PI / 2; bucketBottom.position.y = 0.01; bucketGroup.add(bucketBottom);

  const rim = new THREE.Mesh(new THREE.TorusGeometry(0.42, 0.025, 12, 32),
    new THREE.MeshStandardMaterial({ color: 0xe8e8ea, roughness: 0.35, metalness: 0.2 }));
  rim.rotation.x = Math.PI / 2; rim.position.y = 0.55; bucketGroup.add(rim);

  bucketGroup.position.set(0.5, 0, -2.7);
  markMovable(bucketGroup, 'furniture_bucket', 'Bucket','floor');
  scene.add(bucketGroup);
  ...
  const toiletBase = new THREE.Mesh(new THREE.CylinderGeometry(0.36, 0.44, 0.52, 28), porcelainMat);
  toiletBase.position.y = 0.26; toiletBase.castShadow = true; toiletGroup.add(toiletBase);

  const toiletBowl = new THREE.Mesh(new THREE.CylinderGeometry(0.42, 0.36, 0.46, 28), porcelainMat);
  toiletBowl.position.y = 0.62; toiletBowl.castShadow = true; toiletGroup.add(toiletBowl);

  const toiletSeat = new THREE.Mesh(new THREE.TorusGeometry(0.42, 0.04, 12, 24), porcelainMat);
  toiletSeat.rotation.x = Math.PI / 2; toiletSeat.position.y = 0.85; toiletGroup.add(toiletSeat);

  const toiletTank = new THREE.Mesh(new THREE.BoxGeometry(0.58, 0.78, 0.26), porcelainMat);
  toiletTank.position.set(0, 1.02, -0.36); toiletTank.castShadow = true; toiletGroup.add(toiletTank);

  const flushPlate = new THREE.Mesh(new THREE.BoxGeometry(0.26, 0.13, 0.03), chromeMat);
  flushPlate.position.set(0, 1.08, -0.5); toiletGroup.add(flushPlate);

  toiletGroup.position.set(4.8, 0, -3.0);
  markMovable(toiletGroup, 'furniture_toilet', 'Toilet','floor');
  scene.add(toiletGroup);
